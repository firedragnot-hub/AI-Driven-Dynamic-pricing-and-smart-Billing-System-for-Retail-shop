...
from models import db, BusinessConfig, Purchase, Expense, Transaction, Order, Product, GstCategoryMapping, TransactionItem, OrderItem, PurchaseItem
from datetime import datetime
from extensions import dashboard_cache
from routes.auth import get_current_user, JWT_SECRET
...
def calculate_sales_tax_breakdown(sales_record, biz_state):
    ...
    for item in items:
        ...
        gst_rate = item.product.gst_rate if (item.product and hasattr(item.product, 'gst_rate') and item.product.gst_rate is not None) else 18.0
...
def compute_gst_summary_data():
    ...
    pos_total_sales = float(pos_sum[1] or 0.0)
    ...
    order_total_sales = float(order_sum[1] or 0.0)
    ...
    cgst_itc = sum(p.cgst for p in purchases if p.itc_eligible)
    sgst_itc = sum(p.sgst for p in purchases if p.itc_eligible)
    igst_itc = sum(p.igst for p in purchases if p.itc_eligible)
    
    expenses = Expense.query.all()
    cgst_itc += sum(e.cgst for e in expenses if e.itc_eligible)
    sgst_itc += sum(e.sgst for e in expenses if e.itc_eligible)
    igst_itc += sum(e.igst for e in expenses if e.itc_eligible)
    
    total_itc = cgst_itc + sgst_itc + igst_itc
    
    cgst_payable = max(0.0, cgst_collected - cgst_itc)
    sgst_payable = max(0.0, sgst_collected - sgst_itc)
    igst_payable = max(0.0, igst_collected - igst_itc)
    net_payable = cgst_payable + sgst_payable + igst_payable

    hsn_list = list(hsn_wise.values())
    ...
    hsn_list = sorted(hsn_list, key=lambda x: x['total_amount'], reverse=True)
...
def gst_pnl():
    ...
    if start_dt:
        ord_q = ord_q.filter(Order.timestamp >= start_dt)
    if end_dt:
        ord_q = ord_q.filter(Order.timestamp <= end_dt)
    orders = ord_q.all()
